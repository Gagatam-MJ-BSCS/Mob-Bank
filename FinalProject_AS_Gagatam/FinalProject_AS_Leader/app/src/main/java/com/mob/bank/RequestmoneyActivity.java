...
public class RequestmoneyActivity extends  AppCompatActivity  { 
				...
				if (tbMobileNumber.getText().toString().equals("") || !(tbMobileNumber.getText().toString().length() == 11)) {
					...
					if (tbAmmount.getText().toString().equals("") || (Double.parseDouble(tbAmmount.getText().toString()) < 1)) {
						...
						if (tbMobileNumber.getText().toString().equals(mobilenumber)) {
							((EditText)tbMobileNumber).setError("You cant sent a money request to your own account.");
						}
						else {
							fbdb.addListenerForSingleValueEvent(new ValueEventListener() {
									...
									for(int _repeat37 = 0; _repeat37 < (int)(accountsListString.size()); _repeat37++) {
									...
									if (accounExsistBoolean) {
										...
										_CreateNotification(sendername, receiverkey, "REQUEST MONEY", mobilenumber + " sent you money request to " + "with the amount of ₱" + ammount + ".");
										_CreateLogs(uid, "You sent a money request to " + receiversnumber + " with the amount of ₱" + ammount + ".");
										tbMobileNumber.setText("");
										tbAmmount.setText("0");
										SketchwareUtil.showMessage(getApplicationContext(), "Request money was sent successfully");
									}
									else {
	...
	public void _design () {
		tbMobileNumber.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b, int c, int d) { this.setCornerRadius(a); this.setStroke(b, c); this.setColor(d); return this; } }.getIns((int)5, (int)2, 0xFF000000, 0xFFFFFFFF));
