...
public class BuyloadActivity extends  AppCompatActivity  { 
	...
	private EditText tbCellphoneNumber;
	...
	private void initialize(Bundle _savedInstanceState) {
		...
		tbCellphoneNumber = (EditText) findViewById(R.id.tbCellphoneNumber);
		...
		btnAccept.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View _view) {
				if (tbCellphoneNumber.getText().toString().equals("") || tbAmount.getText().toString().equals("")) {
					((EditText)tbAmount).setError("The amount is required.");
					((EditText)tbCellphoneNumber).setError("The cellphone number required.");
				}
				else {
					if (tbCellphoneNumber.getText().toString().length() < 11) {
						((EditText)tbCellphoneNumber).setError("The cellphone number is required and must have length of 11.");
					}
					else {
						if (Double.parseDouble(tbAmount.getText().toString()) < 1) {
							((EditText)tbAmount).setError("The amount is required and must be greater than 1.");
						}
						else {
							uid = FirebaseAuth.getInstance().getCurrentUser().getUid();
							amount = tbAmount.getText().toString();
							cpnumber = tbCellphoneNumber.getText().toString();
							fbdb.addListenerForSingleValueEvent(new ValueEventListener() {
								...
								public void onDataChange(DataSnapshot _dataSnapshot) {
									...
									if (Double.parseDouble(balance) >= Double.parseDouble(amount)) {
										newbalance = String.valueOf(Double.parseDouble(balance) - Double.parseDouble(amount));
										fbdb.child(uid).child("balance").setValue(newbalance);
										_CreateLogs(uid, "You buy a regular load, Cellphone number: " + cpnumber + " Ammount: ₱" + amount + "."  );
										_dialog("Success", "Your account balance will be reduced by the amount you input.");
										tbAmount.setText("0");
									}
									else {
										SketchwareUtil.showMessage(getApplicationContext(), "Your balance is not enough to buy the a load.");
									}
								}
								@Override
	...
	public void _design () {
		tbCellphoneNumber.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b, int c, int d) { this.setCornerRadius(a); this.setStroke(b, c); this.setColor(d); return this; } }.getIns((int)5, (int)2, 0xFF000000, 0xFFFFFFFF));
