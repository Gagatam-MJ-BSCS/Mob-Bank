...
public class PaybillsActivity extends  AppCompatActivity  { 
	...
	private EditText tbCompanyName;
	private TextView textview2;
	private EditText tbReference;
	...
	private void initialize(Bundle _savedInstanceState) {
		...
		tbCompanyName = (EditText) findViewById(R.id.tbCompanyName);
		textview2 = (TextView) findViewById(R.id.textview2);
		tbReference = (EditText) findViewById(R.id.tbReference);
		...
		btnAccept.setOnClickListener(new View.OnClickListener() {
			@Override
			public void onClick(View _view) {
				if (tbReference.getText().toString().equals("") || (tbAmmount.getText().toString().equals("") || tbCompanyName.getText().toString().equals(""))) {
					((EditText)tbCompanyName).setError("The company name is required.");
					((EditText)tbAmmount).setError("The amount is required.");
					((EditText)tbReference).setError("The account no. is required.");
				}
				else {
					if (tbReference.getText().toString().length() < 11) {
						((EditText)tbReference).setError("The account no. is required and must have length of 11.");
					}
					else {
						if (Double.parseDouble(tbAmmount.getText().toString()) < 1) {
							((EditText)tbAmmount).setError("The amount is required and must be greater than 1.");
						}
						else {
							uid = FirebaseAuth.getInstance().getCurrentUser().getUid();
							companyname = tbCompanyName.getText().toString();
							ammount = tbAmmount.getText().toString();
							reference = tbReference.getText().toString();
							fbdb.addListenerForSingleValueEvent(new ValueEventListener() {
								...
								public void onDataChange(DataSnapshot _dataSnapshot) {
									...
									if (Double.parseDouble(balance) >= Double.parseDouble(ammount)) {
										...
										tbAmmount.setText("");
										_CreateLogs(uid, "You paid a bills on " + companyname + " with reference " + reference + " amount ₱" + ammount + ".");
										_dialog("Success", "Your account balance will be reduced by the amount you input.");
									}
									else {
										SketchwareUtil.showMessage(getApplicationContext(), "Your balance is not enough to pay this bills. ");
									}
								}
								@Override
	...
	public void _design () {
		tbCompanyName.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b, int c, int d) { this.setCornerRadius(a); this.setStroke(b, c); this.setColor(d); return this; } }.getIns((int)5, (int)2, 0xFF000000, 0xFFFFFFFF));
		tbReference.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b, int c, int d) { this.setCornerRadius(a); this.setStroke(b, c); this.setColor(d); return this; } }.getIns((int)5, (int)2, 0xFF000000, 0xFFFFFFFF));
		tbAmmount.setBackground(new GradientDrawable() { public GradientDrawable getIns(int a, int b, int c, int d) { this.setCornerRadius(a); this.setStroke(b, c); this.setColor(d); return this; } }.getIns((int)5, (int)2, 0xFF000000, 0xFFFFFFFF));
	}
	...
	public void _CreateNotification (final String _name, final String _to, final String _title, final String _message) {
		...
		createNotificationMap.put("title", _message);
